...
for (i in 1:length(sces)) {
    ...
    if (ncol(rowData(sces[[i]])) >= 3) {
        colnames(rowData(sces[[i]]))[1:3] <- c("name", "type", "value")
    } else if (ncol(rowData(sces[[i]])) == 2) {
        colnames(rowData(sces[[i]]))[1:2] <- c("name", "type")
        rowData(sces[[i]])$value <- NA_character_
    } else {
        colnames(rowData(sces[[i]]))[1] <- "name"
        rowData(sces[[i]])$type  <- NA_character_
        rowData(sces[[i]])$value <- NA_character_
    }
