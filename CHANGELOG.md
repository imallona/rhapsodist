...
- Pinned the self-compiled kallisto to `v0.52.0` (was tracking upstream master, which is not reproducible). bustools is no longer compiled: the self-compiled bustools aborted with `std::bad_alloc` in bustools correct on the CI runner, so the pinned bioconda bustools (`0.45.1`, already in the kallisto conda env) is used instead. kallisto stays self-compiled because the bioconda kallisto needs CPU instructions (AVX) that some machines lack and aborts with "Illegal instruction".
